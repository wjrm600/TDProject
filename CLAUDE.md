...
- 현재 로스터 20종(**0~10 실캐릭터 11** + 플레이스홀더 9(Unit12~20)). **0=Kwang(대검)·1=Greystone(검+방패)·2=Grux(야수 양손)·3=Crunch(맨손)·4=Aurora(얼음 근접)·5=Serath(검/날개)·6=Shinbi(늑대 소환)·7=SunWukong(내부 Wukong, 봉)·8=Terra(대검)·9=Yin(사슬검)·10=Sparrow(궁수, 원거리)** = Epic Paragon 네이티브 메시/애니 **리타깃 없이 직접 사용**(무기 = 메시 내장 본 → 소켓 부착 불필요, 맨손/근접형은 부착 이슈 자체 無). 전부 풀스킬(Q/W/E/R), `build_paragon_character.py` 로 편입. **벤픽 14 도달엔 3 더 필요**(원거리 풀: Belica/Murdock/Revenant/Boris — 전부 `.gitignore`). ⚠️ 팩 내부 폴더명 함정: LtBelica→Belica, SunWukong→Wukong.
