...
- 현재 로스터 20종(**0~13 실캐릭터 14** + 플레이스홀더 6(Unit15~20)) — **벤픽 14(밴4+픽10) 요건 달성**. **0=Kwang(대검)·1=Greystone(검+방패)·2=Grux(야수 양손)·3=Crunch(맨손)·4=Aurora(얼음 근접)·5=Serath(검/날개)·6=Shinbi(늑대 소환)·7=SunWukong(내부 Wukong, 봉)·8=Terra(대검)·9=Yin(사슬검)·10=Sparrow(궁수)·11=LtBelica(내부 Belica)·12=Murdock·13=Revenant** (10~13 = 원거리 히트스캔) = Epic Paragon 네이티브 메시/애니 **리타깃 없이 직접 사용**(무기 = 메시 내장 본 → 소켓 부착 불필요, 맨손/근접형은 부착 이슈 자체 無). 전부 풀스킬(Q/W/E/R), `build_paragon_character.py` 로 편입. (미편입 원거리 = Boris 1종만 `.gitignore` 잔존.) ⚠️ 팩 내부 폴더명 함정: LtBelica→Belica, SunWukong→Wukong. ⚠️ **additive idle 함정**: 히어로 `Idle` 이 additive 면(예: Belica) BS 스케일 왜곡 → 비가산 idle 사용.
- **원거리 = 히트스캔(발사체 불필요)**: 데미지는 위치가 아니라 **타겟에 직접 적용**(`GA_Attack` 거리/트레이스 체크 없음). AI는 `GetEffectiveAttackRange()`(=캐릭터 `AttackRange` 속성) 에서 멈춰 공격. 즉 **원거리 유닛 = `AttackRange` 큰 스탯 행 + 발사 애니**(C++/리빌드 0). Sparrow 는 `DT_CharacterAttributes` 의 **`Ranged` 행(AttackRange 900)** 사용(`BP_Char.AttributeInitRowName='Ranged'`, 스크립트 config `attr_row`). 날아가는 투사체는 순수 VFX(선택적 후속). 로코모션은 **strafe 애셋 미사용 통일**(`Idle`(또는 `Idle_Combat`)+`Jog_Fwd/Bwd/Left/Right`, 순수 strafe 클립 없는 히어로도 동일 규칙). 플레이스홀더 15(Unit6~20)는 char1(구 Alex) 메시 + `ABP_Alex` 재활용. **새 고유 캐릭터 = Paragon 히어로 Fab 임포트 → `BP_Char_Kwang` 복제(메시/ABP/몽타주만 교체)** 가 골든 경로. (구 Alex 세트(BP_Char_Alex·BP_GA_Alex_*·AM_Alex_*)는 **삭제 불가 백본** — 플레이스홀더 15(Unit6~20)의 스킬/몽타주 + `BP_Character` 부모의 기본공격 몽타주 + `DT_Items` 추천이 참조. 로스터에서만 제거됨. 플레이스홀더를 진짜 캐릭터로 채울 때 자연 해소.)
...
- [`Guides/03_Implementation/PARAGON_CHARACTER_PIPELINE.md`](Guides/03_Implementation/PARAGON_CHARACTER_PIPELINE.md) — **고유 캐릭터 양산 가이드** (Paragon 네이티브 + `build_paragon_character.py`)
- [`Guides/03_Implementation/SKILL_AUTHORING_GUIDE.md`](Guides/03_Implementation/SKILL_AUTHORING_GUIDE.md) / [`AI_3D_ASSET_PIPELINE.md`](Guides/03_Implementation/AI_3D_ASSET_PIPELINE.md)(구 리타깃/Meshy, historical)
