# PROJECT_REFERENCE — 프로젝트 상세 레퍼런스

> 이 문서는 과거 비대해진 `CLAUDE.md` 의 **전체 상세 내용을 옮겨온 깊은 레퍼런스**입니다 (2026-06-19 슬림화 시 분리).
> `CLAUDE.md` 는 이제 매 턴 로드되는 "필수 운영 규칙 + 핵심 아키텍처 + live 함정"의 슬림 버전이고,
> 여기에는 **GAS Phase별 마이그레이션 기록 · StateTree task/condition 상세 · 애니메이션 시스템 상세 ·
> 벤픽/상점 UI 저작 계약 · 경제 시스템 상세 · 해결된 historical 이슈** 등 "필요할 때 찾아보는" 내용을 담습니다.
>
> ⚠️ 이 문서가 길고 상세하지만 **`CLAUDE.md` 가 단일 진실 공급원의 요약본**입니다. 둘이 어긋나면 CLAUDE.md 우선,
> 그리고 이 문서를 갱신하세요. 깊은 작업(GAS/StateTree/애니/벤픽 내부 수정) 전에는 이 문서의 해당 섹션을 읽으세요.
